...
export default class RoomImageFunction {
  ...
  static uploadImage: Handler = async (req: Request, res: Response, next: NextFunction) => {
    let { roomGroupId, listImageId, listUniqueId } = req.body;
    ...
    else if (!listImageId || !Array.isArray(listImageId)) next(new HTTP400Error('List image id is not valid'));
    else if (!listUniqueId || !Array.isArray(listUniqueId)) next(new HTTP400Error('List image id is not valid'));
    ...
    files.forEach((file, index) => {
      console.log(listImageId[index]);
      console.log(listImageId[index] === undefined);
      if (listImageId[index] === 'undefined') {
        promises.push(googleStorage.uploadFileToGoogleStoragePromise(file, roomGroupId, listUniqueId[index]));
      } else if (listImageId[index] === 'delete') {
        promises.push(RoomImage.repo.delete(listImageId[index]));
      }
    });
