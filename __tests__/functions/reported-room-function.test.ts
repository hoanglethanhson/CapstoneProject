...
describe('Test function reportedRoom', () => {
    ...
    it('Test create reportedRoom with authentication', () => {
        return request
            .post('/reportedRoom')
            .send({userId: 100, roomGroupId: 100, reportContent: 'test create reportedRoom'})
    ...
    it('Test update reportedRoom setting with authentication', () => {
        return request
            .put('/reportedRoom/100')
            .send({userId: 100, roomGroupId: 100, reportContent: 'test update reportedRoom'})
            .set('Authorization', token)
            .set('Accept', 'application/json')
            .then(response => {
                expect(response.status).toBe(200);
