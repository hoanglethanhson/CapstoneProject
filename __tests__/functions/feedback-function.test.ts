...
describe('Test function feedback', () => {
    ...
    it('Test create feedback with authentication', () => {
        return request
            .post('/feedback')
            .send({userId: 100, email: 'test create', content: 'test create feedback'})
    ...
    it('Test update feedback setting with authentication', () => {
        return request
            .put('/feedback/100')
            .send({userId: 100, email: 'test update', content: 'test create update'})
            .set('Authorization', token)
            .set('Accept', 'application/json')
            .then(response => {
                expect(response.status).toBe(200);
    ...
    it('Test update feedback setting without authentication', () => {
        return request
            .put('/feedback/100')
            .send({userId: 100, email: 'test update', content: 'test create feedback'})
            .set('Accept', 'application/json')
            .then(response => {
                expect(response.status).toBe(200);
            });
