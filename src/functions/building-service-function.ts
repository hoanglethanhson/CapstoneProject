...
export default class BuildingServiceFunction {
    ...
    static createMultipleBuildingService: Handler = async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (error[0]) next(new HTTP400Error(JSON.stringify(error)));
            ...
            for (let i = 0; i < body.data.length; i++) {
                const {serviceId, isDelete} = body.data[i];
                const record = await BuildingService.repo.getOneRecord(buildingId, serviceId);

                if (record && isDelete) promise.push(BuildingService.repo.deleteOneRecord(buildingId, serviceId));
                else if (record) promise.push(BuildingService.repo.updateById(buildingId, serviceId, body.data[i]));
