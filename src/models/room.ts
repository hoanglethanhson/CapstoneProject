...
export class RoomRepository extends Repository<Room> {
    async updateById(roomId: any, roomUpdate: any) {
        ...
        if (room) {
            ...
            if (roomUpdate.roomStatus !== undefined) room.roomStatus = roomUpdate.roomStatus;
    ...
    async isUserBeingInGroup(userId: any, roomGroupId: any) {
        let resultArray = [];
    ...
    async isUserCheckedOutGroup(userId: any, roomGroupId: any) {
        let resultArray = [];
        resultArray = await getManager()
            .createQueryBuilder(Room, 'room')
            .select(['*'])
