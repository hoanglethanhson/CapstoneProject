...
export class BuildingRepository extends Repository<Building> {
    ...
    async getBuildingInformation(typeId: number, hostId: number, isUpdate: boolean) {
        const buildQuery = this.createQueryBuilder('building')
        ...
        if (isUpdate) return buildQuery.getMany();
        else return buildQuery.andWhere('room.room_status <> :roomStatus', {
            roomStatus: ConstantValues.ROOM_WAS_DELETED
        }).getMany();
    }

    async getBuildingInformationById(buildingId: number, isGetUpdate: boolean = true) {
        if (isGetUpdate) return this.createQueryBuilder('building')
            .leftJoinAndSelect('building.roomGroups', 'roomGroup')
            .leftJoinAndSelect('roomGroup.rooms', 'room')
            .andWhere('building.id = :buildingId', {buildingId})
            .getOne();
        else return this.createQueryBuilder('building')
            .leftJoinAndSelect('building.roomGroups', 'roomGroup')
            .leftJoinAndSelect('roomGroup.roomAmenities', 'roomAmenities')
            .leftJoinAndSelect('roomGroup.roomImages', 'roomImages')
