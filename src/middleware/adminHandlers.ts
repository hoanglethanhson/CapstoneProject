...
const adminHandlers = (request: Request, response: Response, next: NextFunction) => {
        ...
        FirebaseAdmin.auth().verifyIdToken(accessTokenFromClient)
            .then(function (decodedToken) {
                if (request['userAdmin']) {
                    ...
                    request['idToken'] = accessTokenFromClient;
                    next();
                } else {
                    next(new HTTP401Error('You are not authorized to perform this action!'));
                }
